...
- =zpools=: Set of ZFS pools
  ...
  - =datasets=: Dataset configuration
    - =name= (string): Name of the dataset.
      Default: pool name/entry name

    - =mountpoint= (null or path): Mount point of the dataset.
      Default: name without first component (e.g. =nixos/nix= would be mounted at =/nix=)

    - =options=: (set of strings): Options of the dataset.
      Default: empty set

    - =mountOpts= (list of strins): Mount options of the dataset.
...
- =tpmDecrypt= (bool): Should this device be decrypted using the TPM?
  If set, automatically enrolls an empty TPM2 decryption policy.
  Default: false

- =tpmMeasure= (bool): Should the LUKS identity be measured into PCR 15?
  This should be set for the root partition to lock the decryption key
  after it has been retrieved from the TPM.
  Default: false

- =fixate= (null or string): Expect this volume key hash during decryption.
  See =crypttab(5)=, =fixate-volume-key= for more information.
  Default: null
