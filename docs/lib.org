...
- =importTree=: A function to recursively import *all* Nix files in a directory.
  Takes two parameters:
  - a directory path
  - an exclusion filter, which can be:
    - a predicate returning true if a path should be excluded
    - a list of excluded paths
    - or anything else (e.g. ~null~) to disable this function
    Paths are given relative to the imported directory with a leading slash.
