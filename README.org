...
*** Options
- =aquaris.machine.secretKey=: Location of the SSH secret key;
  should be changed when using an ephemeral root storage
  (default: =/etc/ssh/ssh_host_ed25519_key=)
- =aquaris.machine.keyMap=: Key map of the system consoles (default =de-latin1=)
- =aquaris.machine.locale=: System locale (default =en_US.UTF-8=)
- =aquaris.machine.timeLocale=: Locale to display the time in (LC_TIME) (default =de_DE.UTF-8=)
- =aquaris.machine.timeZone=: Time zone of the system (default =Europe/Berlin=)
...
** [[file:persistence.nix][persistence]]
...
*** Options
- =aquaris.persistence.system=: List of persistent system paths.
  Default entries: =/etc/secureboot=, =/var/db/sudo=, =/var/log=
- =aquaris.persistence.users.<name>=:
